import { PlayerData, DEFAULT_PLAYER } from './models/player';
import { saveLocal, loadLocal } from './utils/storage';

App<{
  globalData: {
    player: PlayerData | null;
    stepsAuthorized: boolean;
  ...
  savePlayerData(data: Partial<PlayerData>): Promise<void>;
}>({
  globalData: {
    player: null,
  ...
  async loadPlayerData(): Promise<PlayerData> {
    ...
    if (this.globalData.cloudReady) {
      try {
        ...
        this.globalData.player = res.data as PlayerData;
        // 同步到本地缓存
  ...
  async savePlayerData(data: Partial<PlayerData>) {
