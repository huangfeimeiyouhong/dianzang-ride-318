...
export type GamePhase = 'find' | 'hold' | 'done';

export interface AvalancheCallbacks {
  onPhaseChange: (phase: GamePhase) => void;
  onTimerChange: (sec: number) => void;
  onHoldChange: (sec: number) => void;
...
// 难度影响安全区大小和时限
const DIFF_CONFIG: Record<number, { timeMult: number; zoneRadiusMult: number }> = {
...
export class AvalancheGame extends BaseGame {
  ...
  private snowY = -50;
  private snowSpeed = 2;
  private phase: GamePhase = 'find';
  private elapsed = 0;
  private holdTimer = 0;
  private inSafeZone = false;
  private activeZone = -1;
  private callbacks: AvalancheCallbacks;
  private particles: { x: number; y: number; vy: number; size: number; alpha: number }[] = [];
  private totalTime: number;
  private zoneRadiusMult: number;

  constructor(canvas: any, w: number, h: number, dpr: number, cb: AvalancheCallbacks, difficulty: number = 2) {
