...
class ClimbChargeGame extends BaseGame {
  constructor(canvas, width, height, dpr, callbacks) {
    super(canvas, width, height, dpr);
    this.callbacks = callbacks;
    this.charge = 0;
    this.charging = false;
    this.chargeSpeed = 0.5;
    this.targetCharge = 80;
    this.phase = 'charge';
    this.bikeY = 0;
  ...
  onRender() {
    ...
    if (this.phase === 'charge' || this.phase === 'release') {
      ...
      if (this.charge >= this.targetCharge) fillColor = this.colors.chargeFull;
      else if (this.charge >= 50) fillColor = this.colors.chargeGood;
...
module.exports = { ClimbChargeGame };
