...
function getWaveConfig(difficulty) {
...
export class RockDodgeGame extends BaseGame {
    constructor(canvas, w, h, dpr, callbacks, difficulty = 2) {
        super(canvas, w, h, dpr);
        this.playerLane = 1; // 当前车道（0/1/2）
        this.targetLane = 1;
        this.playerX = 0;
        this.playerY = 0;
        this.laneWidth = 0;
    ...
    swipe(dir) {
        if (dir === 'left')
            this.targetLane = Math.max(0, this.playerLane - 1);
        else
            this.targetLane = Math.min(LANES - 1, this.playerLane + 1);
    }
    onUpdate(dt) {
        if (this.gameOver)
            return;
        ...
        for (let i = this.rocks.length - 1; i >= 0; i--) {
            ...
            // 碰撞检测
            const playerSize = 30;
            if (rectCollision(this.playerX - playerSize / 2, this.playerY - playerSize / 2, playerSize, playerSize, rock.x - rock.size / 2, rock.y - rock.size / 2, rock.size, rock.size)) {
