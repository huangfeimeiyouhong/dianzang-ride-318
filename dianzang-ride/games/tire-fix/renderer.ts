...
interface PuncturePoint {
  x: number;
  y: number;
  fixed: boolean;
  fixAnim: number;   // 修补动画进度 0~1
}
...
// 难度配置
const DIFF_CONFIG: Record<number, { punctureCount: number; timeMult: number }> = {
...
export interface TireFixCallbacks {
  onFixedChange: (fixed: number, total: number) => void;
  onTimerChange: (sec: number) => void;
  onFinish: (won: boolean) => void;
}

export class TireFixGame extends BaseGame {
  private punctures: PuncturePoint[] = [];
  private elapsed = 0;
  private fixedCount = 0;
  private tireCenter: { x: number; y: number } = { x: 0, y: 0 };
  private tireRadius = 0;
  private callbacks: TireFixCallbacks;
  private sparkles: { x: number; y: number; life: number; vx: number; vy: number }[] = [];
  private punctureCount: number;
  private timeLimit: number;

  constructor(canvas: any, w: number, h: number, dpr: number, cb: TireFixCallbacks, difficulty: number = 2) {
