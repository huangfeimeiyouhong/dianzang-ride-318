export interface Equipment {
  ...
  icon: string;
  desc: string;
  type: 'bike' | 'gear' | 'consumable';
  effect: EquipmentEffect;
  obtainMethod: string;
  price?: number;         // 金币价格（商店购买）
  unlockStation?: number; // 在哪个站点解锁
}

export interface EquipmentEffect {
  flatSpeedBonus?: number;     // 平路效率加成（倍率）
  energyCostReduction?: number; // 体能消耗减免（百分比）
  immuneEvent?: string;        // 免疫某事件 ID
  rockDmgReduction?: number;   // 落石伤害减免（百分比）
  energyRestore?: number;      // 恢复体能值
  moodRestore?: number;        // 恢复心情值
  adaptBoost?: number;         // 高原适应度提升
  weatherImmune?: boolean;     // 免疫天气惩罚
  repairDurability?: boolean;  // 修复装备耐久
}
