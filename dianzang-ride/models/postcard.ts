...
export interface Postcard {
  id: string;           // 对应站点 ID: station_{index}
  stationIndex: number; // 站点索引
  stationName: string;  // 站点名称
  title: string;        // 明信片标题
  desc: string;         // 明信片描述
  emoji: string;        // 明信片图标（用作占位图）
  rarity: 'common' | 'rare' | 'legendary'; // 稀有度
}

/** 14张明信片，每站一张 */
export const POSTCARD_LIST: Postcard[] = [
  {
...
/** 获取明信片定义 */
export function getPostcard(stationIndex: number): Postcard | undefined {
  return POSTCARD_LIST.find(p => p.stationIndex === stationIndex);
}

/** 获取稀有度配置 */
export function getRarityConfig(rarity: Postcard['rarity']) {
  const configs = {
