...
import { STATIONS, Station, getStationPair } from '../../models/station';
import { RidersByStation } from '../../models/rider';
import { fetchRiders, updateMyPosition } from '../../services/rider-service';
import { syncDailySteps } from '../../services/step-service';
import { checkDailyLoginReward, claimDailyReward, getRewardCycle, DailyReward } from '../../services/daily-reward-service';
import { checkAndRefreshQuests, QuestProgress, claimQuestReward } from '../../services/quest-service';
import { getTodayWeather, Weather } from '../../services/weather-service';
...
Page({
  data: {
    ...
    currentStation: 0,
    progressToNext: 0,
    stations: [] as StationView[],

    // 每日任务
    quests: [] as QuestProgress[],
    showQuests: false,

    // 登录奖励弹窗
    showLoginReward: false,
    loginReward: null as DailyReward | null,
    loginStreak: 0,
    rewardCycle: [] as DailyReward[],
    loginIsReset: false,
  ...
  refreshUI() {
    ...
    // 构建站点列表数据
    const stations: StationView[] = STATIONS.map((s) => {
      const riders = this._riders[s.id] || [];
      return {
        ...s,
        riderCount: riders.length,
        riderAvatars: riders.slice(0, 3).map(() => '🚴'),
      };
    });
  ...
  onClaimQuest(e: WechatMiniprogram.TouchEvent) {
    const questId = e.currentTarget.dataset.id as string;
    const player = this._player;
    if (!player) return;
  ...
  async syncSteps() {
    ...
    if (!player) return;
    const app = getApp();
    if (!app.globalData.stepsAuthorized) return;
    try {
  ...
  // ========== 路线交互 ==========
  onStationTap(e: WechatMiniprogram.TouchEvent) {
