...
import { PlayerData, EquipmentItem } from '../models/player';
import { Equipment, EQUIPMENT_LIST, getEquipment } from '../models/equipment';

/** 获取玩家已装备列表（完整信息） */
export function getPlayerEquipment(player: PlayerData): (Equipment & { durability: number })[] {
  return player.equipment
    .map(e => {
      const def = getEquipment(e.id);
      if (!def) return null;
      return { ...def, durability: e.durability };
    })
    .filter(Boolean) as (Equipment & { durability: number })[];
}

/** 获取当前站点可购买的装备 */
export function getShopItems(stationIndex: number): Equipment[] {
  return EQUIPMENT_LIST.filter(e => {
    if (!e.price) return false;
    if (e.unlockStation !== undefined && e.unlockStation > stationIndex) return false;
    return true;
  });
}

/** 购买装备 */
export function buyEquipment(player: PlayerData, equipId: string): { success: boolean; message: string } {
  const equip = getEquipment(equipId);
  if (!equip || !equip.price) return { success: false, message: '无效装备' };
  ...
  if (equip.type !== 'consumable') {
    ...
    if (owned) return { success: false, message: '已拥有此装备' };
  }
...
/** 使用消耗品 */
export function useConsumable(player: PlayerData, itemId: string): { success: boolean; message: string } {
  const item = player.inventory.items.find(i => i.id === itemId);
  if (!item || item.count <= 0) return { success: false, message: '物品不足' };

  const equip = getEquipment(itemId);
  if (!equip) return { success: false, message: '无效物品' };
...
/** 降低装备耐久（所有装备，非仅自行车） */
export function degradeEquipment(player: PlayerData, amount: number): void {
  for (const eq of player.equipment) {
...
export function getDurabilityPenalty(player: PlayerData): number {
  const minDur = player.stats.durability;
  if (minDur < 20) return 1.5;   // 严重磨损：消耗 x1.5
  if (minDur < 50) return 1.2;   // 轻度磨损：消耗 x1.2
  return 1.0;
}

/** 检查耐久是否过低（触发警告） */
export function isDurabilityLow(player: PlayerData): boolean {
  return player.stats.durability < 30;
}

/** 检查是否有免疫某事件的装备 */
export function isImmuneToEvent(player: PlayerData, eventId: string): boolean {
  return player.equipment.some(eq => {
    const def = getEquipment(eq.id);
    return def?.effect.immuneEvent === eventId && eq.durability > 0;
  });
}
