...
/** 根据站点索引计算难度等级 1-4 */
export function getDifficulty(stationIndex) {
    if (stationIndex <= 3)
        return 1; // 简单
    if (stationIndex <= 7)
        return 2; // 普通
...
/** 尝试触发随机事件 */
export function tryTriggerEvent(player) {
    ...
    const candidates = EVENT_POOL.filter(e => {
        // 新手保护：只触发正面事件
        if (isBeginner && e.penalties.energy && e.penalties.energy < -5)
            return false;
        // 里程限制
        if (progress.km < e.minKm)
            return false;
        // 区段限制
        if (e.zones !== 'all' && !e.zones.includes(progress.currentStation))
            return false;
        // 装备免疫
        const isImmune = equipment.some(eq => {
            if (eq.id === 'puncture_proof_tire' && e.id === 'flat_tire')
                return true;
            if (eq.id === 'dog_repeller' && e.id === 'dog')
                return true;
            return false;
        });
        if (isImmune)
...
/** 应用事件奖励 */
export function applyReward(player, event) {
    const r = event.rewards;
    if (r.energy)
        player.stats.energy = Math.min(200, player.stats.energy + r.energy);
    if (r.coins)
        player.inventory.coins += r.coins;
    if (r.mood)
        player.stats.mood = Math.min(100, player.stats.mood + r.mood);
    // 物品奖励：添加到背包
    if (r.item) {
        const itemMap = {
...
/** 应用事件惩罚 */
export function applyPenalty(player, event) {
    ...
    if (p.energy)
        player.stats.energy = Math.max(0, player.stats.energy + Math.round(p.energy * dmgMultiplier));
    if (p.mood)
        player.stats.mood = Math.max(0, player.stats.mood + p.mood);
    if (p.durability)
        player.stats.durability = Math.max(0, player.stats.durability + p.durability);
    // 里程倒退
    if (p.kmBack) {
        const stationKm = event.penalties.kmBack || 0;
        player.progress.km = Math.max(STATIONS[player.progress.currentStation].km, player.progress.km - stationKm);
    }
    // 高原反应状态标记（雪崩等触发）
    if (p.triggerHighAlt) {
