...
export type QuestType = 'steps' | 'ride' | 'event' | 'social' | 'equip';
...
export function generateDailyQuests(): QuestProgress[] {
...
/** 检查并刷新今日任务 */
export function checkAndRefreshQuests(player: PlayerData): QuestProgress[] {
  const today = getTodayStr();

  // 读取存储的任务
  const stored = (player.progress as any).dailyQuests;
  const storedDate = (player.progress as any).dailyQuestDate;
  ...
  (player.progress as any).dailyQuests = quests;
  (player.progress as any).dailyQuestDate = today;
...
export function updateQuestProgress(
  ...
  type: QuestType,
  amount: number
): { updated: boolean; newlyCompleted: QuestProgress[] } {
...
/** 领取任务奖励 */
export function claimQuestReward(player: PlayerData, questId: string): { success: boolean; coins: number } {
  const quests = checkAndRefreshQuests(player);
