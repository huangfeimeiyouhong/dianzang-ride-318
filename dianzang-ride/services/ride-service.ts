...
export interface RideResult {
  actualKm: number;       // 实际骑行公里数
  energyCost: number;     // 消耗的体能
  costMultiplier: number; // 最终消耗系数
  reachedNext: boolean;   // 是否到达下一站
  newStation: number;     // 到达的站点 index
  coinsEarned: number;    // 本次骑行获得的金币
}

const MAX_KM_PER_RIDE = 80; // 单次骑行上限

/** 计算海拔消耗系数 */
function getAltitudeCost(elevation: number): number {
  if (elevation > 4000) return 1.5;
  if (elevation > 3000) return 1.2;
  return 1.0;
}

/** 计算每体能点推进的公里数 */
function getKmPerEnergy(elevation: number): number {
  if (elevation > 3500) return 0.5;
  if (elevation > 3000) return 1.0;
  return 2.0;
}

/** 执行一次骑行计算 */
export function calculateRide(player: PlayerData): RideResult {
...
/** 获取当前路段描述文字 */
export function getTerrainDesc(elevation: number): string {
  if (elevation > 4000) return '极高海拔，呼吸困难';
  if (elevation > 3500) return '高海拔，体能消耗加倍';
  if (elevation > 3000) return '高原路段，注意高反';
  return '平缓路段，适合骑行';
}
