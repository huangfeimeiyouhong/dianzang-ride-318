...
import { RiderPosition, RidersByStation } from '../models/rider';
...
/** 生成模拟骑友数据 */
function generateMockRiders(): RidersByStation {
  const result: RidersByStation = {};
  ...
  for (let i = 0; i < riderCount; i++) {
    ...
    if (!result[stationIndex]) result[stationIndex] = [];
...
/** 获取路线上全部可见骑友（按站点分组） */
export async function fetchRiders(): Promise<RidersByStation> {
  const app = getApp();
  ...
  try {
    ...
    return (res.result as { riders: RidersByStation }).riders;
  } catch (e) {
...
/** 更新自己在路线上的位置 */
export async function updateMyPosition(player: PlayerData): Promise<void> {
  const app = getApp();
  const cloudReady = app?.globalData?.cloudReady;
  ...
  const posData: Omit<RiderPosition, '_id'> = {
    nickname: player.nickname,
    rider: player.rider,
    title: player.rider?.title || '骑手',
  ...
  try {
    ...
    console.log('[RiderService] 位置同步跳过（云函数不可用）:', e?.message);
  }
}
...
export async function setVisibility(visible: boolean): Promise<void> {
  ...
  try {
    ...
    await (db.collection('rider_positions') as any)
      .where({ _openid: (db as any).currentUser?.openid || '' })
      .update({ data: { visible } });
  } catch (e: any) {
    // 本地调试时静默跳过
    console.log('[RiderService] 隐身设置跳过（云端不可用）:', e?.message);
  }
}

/** 获取某站点的骑友数量（轻量查询） */
export async function getStationRiderCount(stationIndex: number): Promise<number> {
  const app = getApp();
  const cloudReady = app?.globalData?.cloudReady;
