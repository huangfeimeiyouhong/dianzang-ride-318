...
/** 获取站点对应的特色事件 */
export function getSpecialEvent(stationIndex) {
    return SPECIAL_EVENTS.find(e => e.stationIndex === stationIndex);
}
/** 检查特色事件是否已完成 */
export function isSpecialEventCompleted(player, eventId) {
    const completed = player.completedSpecialEvents || [];
...
export function completeSpecialEvent(player, eventId, choiceIndex) {
    ...
    if (reward.energy)
        player.stats.energy = Math.min(200, player.stats.energy + reward.energy);
    if (reward.mood)
        player.stats.mood = Math.min(100, player.stats.mood + reward.mood);
    if (reward.adaptBoost)
        player.stats.altitudeAdapt = Math.min(50, player.stats.altitudeAdapt + reward.adaptBoost);
    if (reward.title)
        player.rider.title = reward.title;
    // 应用物品奖励
    if (reward.item) {
    ...
    if (penalty) {
        if (penalty.energy)
            player.stats.energy = Math.max(0, player.stats.energy + penalty.energy);
        if (penalty.mood)
            player.stats.mood = Math.max(0, player.stats.mood + penalty.mood);
    }
    // 标记已完成
    const completed = player.completedSpecialEvents || [];
    if (!completed.includes(eventId)) {
        completed.push(eventId);
        player.completedSpecialEvents = completed;
    }
    // 构建奖励描述
    const parts = [];
    if (reward.coins)
        parts.push(`+${reward.coins}金币`);
    if (reward.energy)
        parts.push(`+${reward.energy}体能`);
    if (reward.mood)
        parts.push(`+${reward.mood}心情`);
    if (reward.adaptBoost)
        parts.push(`+${reward.adaptBoost}适应度`);
    if (reward.itemName)
        parts.push(`+${reward.itemName}`);
    if (reward.title)
        parts.push(`称号:${reward.title}`);
    if (penalty === null || penalty === void 0 ? void 0 : penalty.energy)
        parts.push(`${penalty.energy}体能`);
    if (penalty === null || penalty === void 0 ? void 0 : penalty.mood)
        parts.push(`${penalty.mood}心情`);
    return {
        success: true,
        message: parts.join('  '),
        reward,
