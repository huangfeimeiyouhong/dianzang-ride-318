...
export interface StepResult {
  ...
  energy: number;
  coins: number;
  isNew: boolean; // 是否是今天首次同步
}

/** 请求微信运动授权 */
export function requestWeRunAuth(): Promise<boolean> {
...
/** 获取并同步今日步数 */
export async function syncDailySteps(player: PlayerData): Promise<StepResult> {
  const app = getApp();
  ...
  try {
    const weRunRes = await new Promise<WechatMiniprogram.GetWeRunDataSuccessCallbackResult>(
      (resolve, reject) => {
    ...
    if (cloudReady) {
      ...
      try {
        ...
        const cloudResult = res.result as { steps: number; isNew: boolean };
 ...
 */
function getLocalStepsFromWeRun(weRunRes: WechatMiniprogram.GetWeRunDataSuccessCallbackResult): number {
  const stepInfoList = (weRunRes as any).stepInfoList;
 ...
 */
function generateSimulatedSteps(player: PlayerData): number {
...
/** 将步数应用到玩家数据（含体能储备逻辑） */
export function applyStepsToPlayer(player: PlayerData, steps: number, isNew: boolean): StepResult {
...
/** 从储备中提取体能 */
export function useEnergyReserve(player: PlayerData, amount: number): { used: number; message: string } {
  const available = Math.min(amount, player.stats.energyReserve);
  if (available <= 0) return { used: 0, message: '储备体能不足' };
