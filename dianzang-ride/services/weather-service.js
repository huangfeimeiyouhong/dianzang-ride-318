...
/** 生成或获取今日天气 */
export function getTodayWeather(player) {
    const today = getTodayStr();
    // 如果今天已生成过且站点没变，直接返回
    if (player.stats.todayWeather && player.stats.lastWeatherDate === today) {
        return WEATHER_LIST[player.stats.todayWeather];
    }
    // 根据当前海拔调整天气概率
    const station = require('../models/station').STATIONS[player.progress.currentStation];
    const elevation = station ? station.elev : 2000;
    // 高海拔更多雪和暴风雪，低海拔更多雨
    let weights = WEATHER_WEIGHTS.map(w => (Object.assign({}, w)));
    if (elevation > 3000) {
        weights = weights.map(w => {
            if (w.type === 'snow')
                return Object.assign(Object.assign({}, w), { weight: w.weight * 2 });
            if (w.type === 'blizzard')
                return Object.assign(Object.assign({}, w), { weight: w.weight * 2 });
            if (w.type === 'light_rain')
                return Object.assign(Object.assign({}, w), { weight: w.weight * 0.5 });
            if (w.type === 'heavy_rain')
                return Object.assign(Object.assign({}, w), { weight: w.weight * 0.5 });
            return w;
        });
    }
    ...
    // 保存到玩家数据
    player.stats.todayWeather = chosen;
    player.stats.lastWeatherDate = today;
    // 心情影响
    player.stats.mood = Math.max(0, Math.min(100, player.stats.mood + WEATHER_LIST[chosen].moodEffect));
    return WEATHER_LIST[chosen];
}
/** 获取天气对骑行消耗的倍率 */
export function getWeatherCostMult(player) {
    const weather = getTodayWeather(player);
    if (player.stats.weatherImmune)
        return 1.0; // 保暖衣物免疫天气
    return weather.costMult;
}
/** 获取天气对特定事件概率的修正 */
export function getWeatherEventMod(player, eventId) {
    const weather = getTodayWeather(player);
